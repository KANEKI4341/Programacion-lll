    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (File.Exists(rutaArchivo))
            {
                ...
                ///foreach (string linea in lineas.Skip(Math.Max(0, lineas.Length - 3)) ) 
            ...
            Console.WriteLine("Escriba un pensamiento o actividad de su día: ");
            string pensamiento = Console.ReadLine();

            File.AppendAllText(rutaArchivo, $"[{fecha}] - {nombre}: {pensamiento}{Environment.NewLine}");
        }
    }
}
